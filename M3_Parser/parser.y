...
program: declarationsList codeImplementationList        {$$ = astCreate(NULL, AST_FULL_PROG, $1, $2);}
    | declarationsList                                  {$$ = astCreate(NULL, AST_HEADER, $1);}
    ;
...
declaration: dataType TK_IDENTIFIER '=' expression ';'              {$$ = astCreate($2, AST_VAR_DECLARATION, $1, $4);}
    | dataType TK_IDENTIFIER '[' LIT_INT ']' ';'                    {$$ = astCreate($2, AST_ARRAY_DECLARATION, $1, $4);}
    | dataType TK_IDENTIFIER '[' LIT_INT ']' literalsList ';'       {$$ = astCreate($2, AST_ARRAY_DEC_AND_INIT, $1, $4, $6);}        
    | dataType TK_IDENTIFIER '(' formalParameterList ')' ';'        {$$ = astCreate($2, AST_FUNCTION_DECLARATION, $1, $4);}
    ;

formalParameter: dataType TK_IDENTIFIER                 {$$ = astCreate($2, AST_PARAM_LIST, $1);}
    ;
...
expression: literal                                     {$$=$1;}
    ...
    | TK_IDENTIFIER '[' expression ']'                  {$$ = astCreate(NULL, AST_ARRAY_EXPR, $1, $3);}
    | TK_IDENTIFIER '(' argumentsList ')'               {$$ = astCreate(NULL, AST_FUNCTION_CALLING, $1, $3);}    
    ...
    | '~' expression                                    {$$ = astCreate(NULL, AST_NOT,$2);}
    ;
...
codeImplementationList: codeImplementation                      {$$ = $1;}
    | codeImplementation codeImplementationList                 {$$ = astCreate(NULL, AST_FUNCTION_LIST, $1, $2);}
    ;

codeImplementation: KW_CODE TK_IDENTIFIER cmd                   {$$ = astCreate(NULL, AST_FUNCTION_IMPLEMENTATION, $2, $3);}
    ;

cmd: '{' '}'                                                    {$$ = NULL;}
    | ';'                                                       {$$ = NULL;}
    | '{' cmdList '}'                                           {$$ = astCreate(NULL, AST_CMD_BLOCK, $2);}          
    | TK_IDENTIFIER '=' expression ';'                          {$$ = astCreate(NULL, AST_ASSIGNMENT, $1, $3);}
    | TK_IDENTIFIER '[' expression ']' '=' expression ';'       {$$ = astCreate(NULL, AST_ARRAY_ASSIGN, $1, $3, $6);}
...
cmdList: cmd                                                    {$$ = astCreate(NULL, AST_CMD_LIST, $1, NULL);}
